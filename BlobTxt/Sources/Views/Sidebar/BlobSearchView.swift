...
// NSTextField wrapper without the system bezel or focus ring, for custom-styled backgrounds.
...
struct BlobSearchView: View {
    ...
    // Preserved for potential future scoped-search features; currently unused — search always covers the full project
    ...
    private var isBlobOpen: Bool { activeBlobID != nil }
    ...
    var body: some View {
        ...
        .padding(.trailing, 8)
        .onAppear {
            singleBlobOnly = false
        }
        .onChange(of: activeBlobID) { newValue in
            if newValue == nil {
                singleBlobOnly = false
                snippetMatches = []
        ...
        HStack(spacing: 6) {
            ...
            .background(
                RoundedRectangle(cornerRadius: 4)
                    .fill(AppColors.shared.surfaceSunken.opacity(0.6))
            )
                ...
                Image(systemName: "magnifyingglass")
                    ...
                    .background(
                        RoundedRectangle(cornerRadius: 5)
                            .fill(hoverSearch ? AppColors.shared.metaIndication : AppColors.shared.surfaceSunken)
                    )
    ...
    private var resultsContent: some View {
        if singleBlobOnly {
            if snippetMatches.isEmpty {
                ...
                VStack(spacing: 6) {
                    ForEach(snippetMatches) { match in
                        snippetCard(for: match)
                    }
                }
            }
        } else {
            if allBlobResults.isEmpty {
                if !searchQuery.isEmpty {
                    emptyMessage
                }
            } else {
                VStack(spacing: 8) {
                    ForEach(allBlobResults) { result in
                        allBlobResultCard(for: result)
                    }
                }
            }
        }
        ...
        return ZStack(alignment: .bottomTrailing) {
            ...
            .overlay(
                RoundedRectangle(cornerRadius: 8)
                    .stroke(isActive ? AppColors.shared.metaIndication : AppColors.shared.borderCard,
                            lineWidth: isActive ? 2 : 1)
                    .shadow(color: isActive ? AppColors.shared.metaIndication.opacity(0.4) : .clear,
                            radius: 6, x: 0, y: 0)
            )
    ...
    private func highlightedSnippet(for match: ProjectStore.SnippetMatch) -> AttributedString {
        ...
        guard let range = snippet.range(of: searchQuery, options: .caseInsensitive) else {
        ...
        HStack(spacing: 6) {
            ...
            Button(action: { if replaceEnabled { showReplaceConfirm = true } }) {
                Image(systemName: "arrow.2.squarepath")
                    .font(.system(size: 13, weight: .medium))
                    .foregroundColor(
                        !replaceEnabled ? AppColors.shared.textMuted
                        : hoverReplace ? AppColors.shared.surface
                        : AppColors.shared.metaIndication
                    )
                    .frame(width: 28, height: 28)
                    .background(
                        RoundedRectangle(cornerRadius: 5)
                            .fill(hoverReplace && replaceEnabled
                                  ? AppColors.shared.metaIndication
    ...
    // MARK: - Replace confirmation text
    ...
    private func performSearch() {
        ...
        if singleBlobOnly, let blobID = activeBlobID {
            activeSnippetIndex = nil
            snippetMatches = store.searchSnippets(blobID: blobID, in: projectID, query: searchQuery)
            NotificationCenter.default.post(name: .searchAndHighlight, object: searchQuery)
        } else {
            allBlobResults = store.searchBlobs(in: projectID, folderID: nil, query: searchQuery)
        }
    }
    ...
    private func executeReplaceAll() {
        ...
        if singleBlobOnly, let blobID = activeBlobID {
            store.replaceAllInBlobs(blobIDs: [blobID], in: projectID, find: searchQuery, replace: replaceText)
            NotificationCenter.default.post(name: .reloadEditorContent, object: blobID)
        } else {
            let blobIDs = allBlobResults.map { $0.blob.id }
            store.replaceAllInBlobs(blobIDs: blobIDs, in: projectID, find: searchQuery, replace: replaceText)
            if let active = activeBlobID, blobIDs.contains(active) {
