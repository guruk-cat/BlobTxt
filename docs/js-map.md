...
`slugify` and `headingPosForSlug` are exported from `links.js` and reused by `main.js` for fold persistence: a folded heading is identified by its slug (`currentFoldSlugs`), and restored by resolving that slug back to a position (`applyFolds`). Session view state adds no new transport; it rides the existing channels. The `load()` payload carries scroll and a `folds` array (restore), and `getViewState` is a bridge method Swift pulls on demand (capture), returning both folds and scroll in one round-trip (paralleling `getContent`). Swift drives the timing (it pulls on the save/flush path, just before teardown); the JS side only reports and applies. Scroll is pulled the same way rather than pushed continuously (there is no scroll listener).
